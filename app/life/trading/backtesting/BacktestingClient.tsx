              ...
              {showNewSession && (
                      <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                <div style={{ background: cardBg, borderRadius: '16px', border: `1px solid ${border}`, padding: '28px', width: '480px', maxWidth: '90vw' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                                                          <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: textPrimary }}>New Backtest Session</h2>h2>
                                            ...
                                            <div style={{ display: 'grid', gap: '14px' }}>
                                                          <div><label style={labelStyle}>Session Name</label>label><input style={inputStyle} placeholder="e.g. NQ Opening Range April 2026" value={newSessionData.name} onChange={e => setNewSessionData(p => ({ ...p, name: e.target.value }))} /></div>div>
                                                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                                                          <div><label style={labelStyle}>Instrument</label>label><select style={{ ...inputStyle, cursor: 'pointer' }} value={newSessionData.instrument} onChange={e => setNewSessionData(p => ({ ...p, instrument: e.target.value }))}>{INSTRUMENTS.map(i => <option key={i} value={i}>{i}</option>option>)}</select>select></div>div>
                                                                          <div><label style={labelStyle}>Playbook</label>label><select style={{ ...inputStyle, cursor: 'pointer' }} value={newSessionData.playbookId} onChange={e => setNewSessionData(p => ({ ...p, playbookId: e.target.value }))}><option value="">None</option>option>{playbooks.map(pb => <option key={pb.id} value={pb.id}>{pb.name}</option>option>)}</select>select></div>div>
                                                          </div>div>
                                                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                                                          <div><label style={labelStyle}>From Date</label>label><input type="date" style={inputStyle} value={newSessionData.dateFrom} onChange={e => setNewSessionData(p => ({ ...p, dateFrom: e.target.value }))} /></div>div>
                                                                          <div><label style={labelStyle}>To Date</label>label><input type="date" style={inputStyle} value={newSessionData.dateTo} onChange={e => setNewSessionData(p => ({ ...p, dateTo: e.target.value }))} /></div>div>
                                                          </div>div>
                                            </div>div>
                ...
                <div style={{ padding: '0 24px 24px', display: 'grid', gridTemplateColumns: sidebarCollapsed ? '56px 1fr' : '260px 1fr', gap: '16px', transition: 'grid-template-columns 0.2s ease' }}>
                        <div style={{ background: cardBg, borderRadius: '12px', border: `1px solid ${border}`, overflow: 'hidden', height: 'fit-content', minHeight: '200px' }}>
                            ...
                            {loading ? <div style={{ padding: '16px', textAlign: 'center', color: textMuted, fontSize: '12px' }}>Loading...</div>div>
                                  : sessions.length === 0 ? (!sidebarCollapsed && <div style={{ padding: '16px', textAlign: 'center', color: textMuted, fontSize: '12px' }}>No sessions yet.</div>div>)
                                  : sessions.map(s => (
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '16px', alignItems: 'start' }}>
                                                      ...
                                                      <button key={d} onClick={() => setTradeForm(p => ({ ...p, direction: d }))} style={{ flex: 1, padding: '7px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 700, background: tradeForm.direction === d ? (d === 'Long' ? 'rgba(0,242,255,0.15)' : 'rgba(255,80,80,0.15)') : 'transparent', color: tradeForm.direction === d ? (d === 'Long' ? cyan : '#ff5050') : textMuted, transition: 'all 0.15s' }}>
                                                          {d === 'Long' ? '▲' : '▼'} {d}
                                                                          ...
                                                                          <div style={{ marginBottom: '10px' }}><label style={labelStyle}>Contracts</label>label><input type="number" style={inputStyle} min="1" value={tradeForm.contracts} onChange={e => setTradeForm(p => ({ ...p, contracts: e.target.value }))} /></div>div>
                                                            {previewPnl !== null && (
                                                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px', padding: '8px 10px', borderRadius: '8px', background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.03)' }}>
                                                                        ...
                                                                        <div><div style={{ ...labelStyle, marginBottom: '2px' }}>R:R</div>div><div style={{ fontSize: '14px', fontWeight: 700, color: (previewRR || 0) >= 1 ? '#48bb78' : textMuted, fontFamily: 'JetBrains Mono, monospace' }}>{previewRR ? previewRR.toFixed(2) : '—'}</div>div></div>div>
                                                  </div>div>
                                                                          )}
                                                      ...
                                                      <button key={r} onClick={() => setTradeForm(p => ({ ...p, result: r }))} style={{ flex: 1, padding: '6px', borderRadius: '6px', border: `1px solid ${tradeForm.result === r ? (r === 'Win' ? '#48bb78' : r === 'Loss' ? '#fc8181' : '#f6c90e') : border}`, background: tradeForm.result === r ? (r === 'Win' ? 'rgba(72,187,120,0.12)' : r === 'Loss' ? 'rgba(252,129,129,0.12)' : 'rgba(246,201,14,0.12)') : 'transparent', color: tradeForm.result === r ? (r === 'Win' ? '#48bb78' : r === 'Loss' ? '#fc8181' : '#f6c90e') : textMuted, cursor: 'pointer', fontSize: '11px', fontWeight: 700 }}>
                                                          {r}
                                                                          ...
                                                                          <div style={{ marginBottom: '14px' }}><label style={labelStyle}>Notes</label>label><input style={inputStyle} placeholder="Optional notes..." value={tradeForm.notes} onChange={e => setTradeForm(p => ({ ...p, notes: e.target.value }))} /></div>div>
                                                  ...
                                                  <div style={{ overflowX: 'auto' }}>
                                                                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                                                                                  ...
                                                                                  <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: textMuted, fontWeight: 600, fontSize: '11px', whiteSpace: 'nowrap' }}>{h}</th>th>
                                                                                ))}
                                                                                                    </tr>tr></thead>thead>
                                                                                                <tbody>
                                                                                  ...
                                                                                  const rc = t.result === 'Win' ? '#48bb78' : t.result === 'Loss' ? '#fc8181' : '#f6c90e'
                                                                                                                  return (
                                                                                                                                                    <tr key={t.id} style={{ background: t.result === 'Win' ? 'rgba(72,187,120,0.05)' : t.result === 'Loss' ? 'rgba(252,129,129,0.05)' : 'rgba(246,201,14,0.05)', borderBottom: `1px solid ${border}` }}>
                                                                                                                                                                                    ...
                                                                                                                                                                                    <td style={{ padding: '8px 10px' }}><span style={{ padding: '2px 8px', borderRadius: '4px', background: `${rc}20`, color: rc, fontWeight: 700, fontSize: '11px' }}>{t.result}</span>span></td>td>
