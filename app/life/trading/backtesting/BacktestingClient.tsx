...
import { Plus, Trash2, FlaskConical, TrendingUp, TrendingDown, Brain, ChevronRight, ChevronLeft, X } from 'lucide-react'
...
function TradingViewChart({ symbol, isDark }: { symbol: string; isDark: boolean }) {
    ...
    const timeframes = [{ label: '1m', value: '1' }, { label: '5m', value: '5' }, { label: '15m', value: '15' }, { label: '1h', value: '60' }, { label: '1D', value: 'D' }]

  const iframeSrc = `https://www.tradingview.com/widgetembed/?frameElementId=tv_chart&symbol=${encodeURIComponent(symbol)}&interval=${timeframe}&hidesidetoolbar=1&symboledit=1&saveimage=0&toolbarbg=${isDark ? '0a0f1a' : 'f8f9fc'}&theme=${isDark ? 'dark' : 'light'}&style=1&timezone=America%2FNew_York&withdateranges=1&hidevolume=0&locale=en`

  return (
        <div style={{ width: '100%', borderRadius: '12px', overflow: 'hidden', border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.1)'}`, background: isDark ? '#0a0f1a' : '#fff' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.08)'}` }}>
                          <span style={{ color: '#00f2ff', fontWeight: 600, fontSize: '13px', fontFamily: 'JetBrains Mono, monospace' }}>{symbol}</span>span>
...
export default function BacktestingClient() {
  ...
  const [newSession, setNewSession] = useState({ name: '', instrument: 'NQ1!', playbookId: '', dateFrom: '', dateTo: '' })
  ...
  async function fetchSessions() {
        try {
                ...
                if (data.sessions) {
                          ...
                          if (data.sessions.length > 0 && !activeSession) setActiveSession(data.sessions[0])
                }
  ...
  async function createSession() {
        ...
        const res = await fetch('/api/backtesting', {
                ...
                body: JSON.stringify({ action: 'createSession', name: newSession.name, instrument: newSession.instrument, playbookId: newSession.playbookId || null, dateRange: { from: newSession.dateFrom, to: newSession.dateTo } }),
        })
        const data = await res.json()
        if (data.session) {
                ...
                setNewSession({ name: '', instrument: 'NQ1!', playbookId: '', dateFrom: '', dateTo: '' })
        }
  }
  ...
  async function addTrade() {
        ...
        const contracts = Number(tradeForm.contracts) || 1
  ...
  async function getAnalysis() {
        ...
        setAnalyzing(true)
        try {
                const stats = calcStats(activeSession.trades)
                const res = await fetch('/api/backtesting/analyze', {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({ sessionId: activeSession.id, sessionStats: stats, trades: activeSession.trades }),
                })
                const data = await res.json()
                if (data.analysis) {
          ...
          {/* Header */}
                <div style={{ padding: '24px 24px 0' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
                                      ...
                                      <div>
                                                  <h1 style={{ fontSize: '20px', fontWeight: 700, color: textPrimary, margin: 0 }}>Backtesting</h1>h1>
                                                  <p style={{ fontSize: '12px', color: textMuted, margin: 0 }}>Replay setups. Measure your edge. Compare with live trading.</p>p>
                                      </div>div>
                  ...
                  <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <div style={{ background: cardBg, borderRadius: '16px', border: `1px solid ${border}`, padding: '28px', width: '480px', maxWidth: '90vw' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                                                      <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 700 }}>New Backtest Session</h2>h2>
                                                      <button onClick={() => setShowNewSession(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: textMuted }}><X size={18} /></button>button>
                                        </div>div>
                                        <div style={{ display: 'grid', gap: '14px' }}>
                                                      <div>
                                                                      <label style={labelStyle}>Session Name</label>label>
                                                                      <input style={inputStyle} placeholder="e.g. NQ Opening Range April 2026" value={newSession.name} onChange={e => setNewSession(p => ({ ...p, name: e.target.value }))} />
                                                      </div>div>
                                                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                                                      <div>
                                                                                        <label style={labelStyle}>Instrument</label>label>
                                                                                        <select style={{ ...inputStyle, cursor: 'pointer' }} value={newSession.instrument} onChange={e => setNewSession(p => ({ ...p, instrument: e.target.value }))}>
                                                                                          {INSTRUMENTS.map(i => <option key={i} value={i}>{i}</option>option>)}
                                                                                          </select>select>
                                                                      </div>div>
                                                                      <div>
                                                                                        <label style={labelStyle}>Playbook</label>label>
                                                                                        <select style={{ ...inputStyle, cursor: 'pointer' }} value={newSession.playbookId} onChange={e => setNewSession(p => ({ ...p, playbookId: e.target.value }))}>
                                                                                                            <option value="">None</option>option>
                                                                                          {playbooks.map(pb => <option key={pb.id} value={pb.id}>{pb.name}</option>option>)}
                                                                                          </select>select>
                                                                      </div>div>
                                                      </div>div>
                                                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                                                      <div><label style={labelStyle}>From Date</label>label><input type="date" style={inputStyle} value={newSession.dateFrom} onChange={e => setNewSession(p => ({ ...p, dateFrom: e.target.value }))} /></div>div>
                                                                      <div><label style={labelStyle}>To Date</label>label><input type="date" style={inputStyle} value={newSession.dateTo} onChange={e => setNewSession(p => ({ ...p, dateTo: e.target.value }))} /></div>div>
                                                      </div>div>
                                        </div>div>
                                        <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                                                      <button onClick={() => setShowNewSession(false)} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: `1px solid ${border}`, background: 'transparent', color: textMuted, cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>Cancel</button>button>
                                                      <button onClick={createSession} style={{ flex: 2, padding: '10px', borderRadius: '8px', border: 'none', background: cyan, color: '#0a0f1a', cursor: 'pointer', fontSize: '13px', fontWeight: 700 }}>Create Session</button>button>
                                        </div>div>
                            </div>div>
                  </div>div>
              )}
              ...
              <div style={{ padding: '0 24px 24px', display: 'grid', gridTemplateColumns: sidebarCollapsed ? '56px 1fr' : '260px 1fr', gap: '16px', transition: 'grid-template-columns 0.2s ease' }}>
                ...
                {/* Sessions Sidebar */}
                      <div style={{ background: cardBg, borderRadius: '12px', border: `1px solid ${border}`, overflow: 'hidden', height: 'fit-content', minHeight: '200px' }}>
                                <div style={{ padding: '12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${border}` }}>
                                  {!sidebarCollapsed && <span style={{ fontSize: '11px', fontWeight: 700, color: textMuted, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Sessions</span>span>}
                        ...
                        {loading ? (
                      <div style={{ padding: '16px', textAlign: 'center', color: textMuted, fontSize: '12px' }}>Loading...</div>div>
                    ) : sessions.length === 0 ? (
                      !sidebarCollapsed && <div style={{ padding: '16px', textAlign: 'center', color: textMuted, fontSize: '12px' }}>No sessions yet.<br />Create your first session.</div>div>
                    ) : (
                      sessions.map(s => (
                                      <div key={s.id} onClick={() => setActiveSession(s)} style={{ padding: sidebarCollapsed ? '12px 8px' : '10px 12px', cursor: 'pointer', borderBottom: `1px solid ${border}`, background: activeSession?.id === s.id ? 'rgba(0,242,255,0.06)' : 'transparent', transition: 'background 0.15s', position: 'relative' }}>
                                        {sidebarCollapsed ? (
                                                          <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: activeSession?.id === s.id ? cyan : textMuted, margin: '0 auto' }} />
                                                        ) : (
                                                          <>
                                                                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                                                                    <div style={{ flex: 1, minWidth: 0 }}>
                                                                                                                            <div style={{ fontSize: '12px', fontWeight: 600, color: activeSession?.id === s.id ? cyan : textPrimary, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.name}</div>div>
                                                                                                                            <div style={{ fontSize: '11px', color: textMuted, marginTop: '2px' }}>{s.instrument} · {s.trades.length} trades</div>div>
                                                                                                      </div>div>
                                                                                                    <button onClick={e => { e.stopPropagation(); deleteSession(s.id) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: textMuted, opacity: 0.5, padding: '2px', flexShrink: 0 }}><Trash2 size={12} /></button>button>
                                                                              </div>div>
                                                          </>>
                                                        )}
                                      </div>div>
                ...
                {/* Active Session View */}
                      ...
                      <div style={{ background: cardBg, borderRadius: '12px', border: `1px solid ${border}`, padding: '48px', textAlign: 'center' }}>
                                    ...
                                    <p style={{ color: textMuted, fontSize: '14px' }}>Select a session or create a new one to start backtesting.</p>p>
                      </div>div>
                    ) : (
                      <>
                        {/* TradingView Chart */}
                        ...
                        {/* Bottom: Log Form + Trade Table side by side, Coach Shai below */}
                                    <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '16px', alignItems: 'start' }}>
                                      ...
                                      {/* Log Trade Form */}
                                                    <div style={{ background: cardBg, borderRadius: '12px', border: `1px solid ${border}`, padding: '16px' }}>
                                                                      <div style={{ fontSize: '12px', fontWeight: 700, color: textMuted, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '14px' }}>Log Trade</div>div>
                                                ...
                                                <button key={d} onClick={() => setTradeForm(p => ({ ...p, direction: d }))} style={{ flex: 1, padding: '7px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 700, background: tradeForm.direction === d ? (d === 'Long' ? 'rgba(0,242,255,0.15)' : 'rgba(255,80,80,0.15)') : 'transparent', color: tradeForm.direction === d ? (d === 'Long' ? cyan : '#ff5050') : textMuted, transition: 'all 0.15s' }}>
                                                  {d === 'Long' ? '▲' : '▼'} {d}
                                                                      ...
                                                                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                                                                                          <div><label style={labelStyle}>Entry</label>label><input type="number" style={inputStyle} placeholder="0.00" value={tradeForm.entryPrice} onChange={e => setTradeForm(p => ({ ...p, entryPrice: e.target.value }))} /></div>div>
                                                                                          <div><label style={labelStyle}>Exit</label>label><input type="number" style={inputStyle} placeholder="0.00" value={tradeForm.exitPrice} onChange={e => setTradeForm(p => ({ ...p, exitPrice: e.target.value }))} /></div>div>
                                                                      </div>div>
                                                    
                                                                      <div style={{ marginBottom: '10px' }}>
                                                                                          <label style={labelStyle}>Contracts</label>label>
                                                                                          <input type="number" style={inputStyle} min="1" value={tradeForm.contracts} onChange={e => setTradeForm(p => ({ ...p, contracts: e.target.value }))} />
                                                                      </div>div>
                                            ...
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '10px', padding: '8px 10px', borderRadius: '8px', background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.03)' }}>
                                                                  <div><div style={{ ...labelStyle, marginBottom: '2px' }}>Net P&L</div>div><div style={{ fontSize: '14px', fontWeight: 700, color: previewPnl >= 0 ? '#48bb78' : '#fc8181', fontFamily: 'JetBrains Mono, monospace' }}>${previewPnl.toLocaleString()}</div>div></div>div>
                                                                  <div><div style={{ ...labelStyle, marginBottom: '2px' }}>R:R</div>div><div style={{ fontSize: '14px', fontWeight: 700, color: (previewRR || 0) >= 1 ? '#48bb78' : textMuted, fontFamily: 'JetBrains Mono, monospace' }}>{previewRR?.toFixed(2) || '–'}</div>div></div>div>
                                            </div>div>
                                                                      )}
                                                    
                                                      {/* Result Override */}
                                                                      <div style={{ marginBottom: '10px' }}>
                                                                                          <label style={labelStyle}>Result</label>label>
                                                      ...
                                                      {/* Followed Rules */}
                                                                      <div style={{ marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }} onClick={() => setTradeForm(p => ({ ...p, followedRules: !p.followedRules }))}>
                                                                                          <div style={{ width: '16px', height: '16px', borderRadius: '4px', border: `2px solid ${tradeForm.followedRules ? cyan : border}`, background: tradeForm.followedRules ? cyan : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.15s' }}>
                                                                                            {tradeForm.followedRules && <span style={{ color: '#0a0f1a', fontSize: '10px', fontWeight: 900 }}>✓</span>span>}
                                                                                            </div>div>
                                                                                          <span style={{ fontSize: '12px', color: textPrimary }}>Followed Rules</span>span>
                                                                      </div>div>
                                                    
                                                      {/* Notes */}
                                                                      <div style={{ marginBottom: '14px' }}>
                                                                                          <label style={labelStyle}>Notes</label>label>
                                                                                          <input style={inputStyle} placeholder="Optional notes..." value={tradeForm.notes} onChange={e => setTradeForm(p => ({ ...p, notes: e.target.value }))} />
                                                                      </div>div>
                                      ...
                                      {/* Trade Log Table */}
                                                    <div style={{ background: cardBg, borderRadius: '12px', border: `1px solid ${border}`, overflow: 'hidden' }}>
                                                                      <div style={{ padding: '12px 16px', borderBottom: `1px solid ${border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                                          <span style={{ fontSize: '12px', fontWeight: 700, color: textMuted, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Trade Log</span>span>
                                                                                          <span style={{ fontSize: '11px', color: textMuted }}>{activeSession.trades.length} trades</span>span>
                                                                      </div>div>
                                                      {activeSession.trades.length === 0 ? (
                                            <div style={{ padding: '32px', textAlign: 'center', color: textMuted, fontSize: '13px' }}>No trades yet. Log your first trade.</div>div>
                                          ) : (
                                            <div style={{ overflowX: 'auto' }}>
                                                                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                                                                            ...
                                                                            <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: textMuted, fontWeight: 600, fontSize: '11px', whiteSpace: 'nowrap', letterSpacing: '0.04em' }}>{h}</th>th>
                                                                          ))}
                                                                                                                      </tr>tr>
                                                                                            </thead>thead>
                                                                                          <tbody>
                                                                                            {activeSession.trades.map((t, i) => {
                                                                          const rowBg = t.result === 'Win' ? 'rgba(72,187,120,0.05)' : t.result === 'Loss' ? 'rgba(252,129,129,0.05)' : 'rgba(246,201,14,0.05)'
                                                                                                        const resultColor = t.result === 'Win' ? '#48bb78' : t.result === 'Loss' ? '#fc8181' : '#f6c90e'
                                                                                                                                      return (
                                                                                                                                                                      <tr key={t.id} style={{ background: rowBg, borderBottom: `1px solid ${border}` }}>
                                                                                                                                                                                                      <td style={{ padding: '8px 10px', color: textMuted, fontFamily: 'JetBrains Mono, monospace' }}>{i + 1}</td>td>
                                                                                                                                                                                                      <td style={{ padding: '8px 10px', fontWeight: 700, color: t.direction === 'Long' ? cyan : '#ff5050', fontFamily: 'JetBrains Mono, monospace' }}>{t.direction === 'Long' ? '▲' : '▼'} {t.direction}</td>td>
                                                                                                                                                                                                      <td style={{ padding: '8px 10px', fontFamily: 'JetBrains Mono, monospace', color: textPrimary }}>{t.entryPrice.toLocaleString()}</td>td>
                                                                                                                                                                                                      <td style={{ padding: '8px 10px', fontFamily: 'JetBrains Mono, monospace', color: textPrimary }}>{t.exitPrice.toLocaleString()}</td>td>
                                                                                                                                                                                                      <td style={{ padding: '8px 10px', fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, color: t.netPnl >= 0 ? '#48bb78' : '#fc8181' }}>${t.netPnl.toLocaleString()}</td>td>
                                                                                                                                                                                                      <td style={{ padding: '8px 10px', fontFamily: 'JetBrains Mono, monospace', color: textMuted }}>{t.riskReward > 0 ? t.riskReward.toFixed(2) : '–'}</td>td>
                                                                                                                                                                                                      <td style={{ padding: '8px 10px' }}><span style={{ padding: '2px 8px', borderRadius: '4px', background: `${resultColor}20`, color: resultColor, fontWeight: 700, fontSize: '11px' }}>{t.result}</span>span></td>td>
                                                                                                                                                                                                      <td style={{ padding: '8px 10px', textAlign: 'center' }}><span style={{ color: t.followedRules ? '#48bb78' : '#fc8181', fontSize: '14px' }}>{t.followedRules ? '✓' : '✗'}</span>span></td>td>
                                                                                                                                                                                                      <td style={{ padding: '8px 10px', color: textMuted, maxWidth: '120px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.notes || '–'}</td>td>
                                                                                                                                                                                                      <td style={{ padding: '8px 10px' }}><button onClick={() => deleteTrade(t.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: textMuted, opacity: 0.5, padding: '2px' }}><Trash2 size={12} /></button>button></td>td>
                                                                                                                                                                        </tr>tr>
                                                                                                                                                                    )
                                                                                              })}
                                                                                            </tbody>tbody>
                                    ...
                                    <div style={{ background: cardBg, borderRadius: '12px', border: `1px solid ${border}`, overflow: 'hidden' }}>
                                                    <div style={{ padding: '14px 16px', borderBottom: `1px solid ${border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                                                          ...
                                                                                          <span style={{ fontSize: '13px', fontWeight: 700, color: textPrimary }}>Coach Shai Analysis</span>span>
                                                                      </div>div>
                                            ...
                                            <button onClick={getAnalysis} disabled={analyzing} style={{ padding: '7px 16px', borderRadius: '8px', border: 'none', background: analyzing ? 'rgba(0,242,255,0.3)' : cyan, color: '#0a0f1a', fontWeight: 700, fontSize: '12px', cursor: analyzing ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                    ...
                                                    <div style={{ padding: '16px' }}>
                                                      {activeSession.trades.length < 5 ? (
                                            <p style={{ color: textMuted, fontSize: '13px', margin: 0 }}>Log at least 5 trades to unlock Coach Shai analysis comparing your backtest vs live performance.</p>p>
                                          ) : activeSession.coachAnalysis ? (
                                            <div style={{ whiteSpace: 'pre-wrap', lineHeight: '1.7', fontSize: '14px', color: textPrimary }}>{activeSession.coachAnalysis}</div>div>
                                          ) : (
                                            <p style={{ color: textMuted, fontSize: '13px', margin: 0 }}>Click "Get Analysis" to receive personalized insights from Coach Shai comparing your backtest results to live trading performance.</p>p>
                                                                      )}
                                                    </div>div>
                                    </div>div>
                      </>>
