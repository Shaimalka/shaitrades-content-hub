...
interface Debt { id: string; userId: string; name: string; type: 'credit_card' | 'student_loan' | 'personal_loan' | 'mortgage' | 'auto_loan' | 'other'; balance: number; originalBalance: number; interestRate: number; minimumPayment: number; dueDate?: number; createdAt: string; updatedAt: string }
...
function FinancePage() {
  ...
  const [debtForm, setDebtForm] = useState({ name: '', type: 'credit_card' as Debt['type'], balance: '', originalBalance: '', interestRate: '', minimumPayment: '', dueDate: '' })
  ...
  function resetDebtForm() { setDebtForm({ name: '', type: 'credit_card', balance: '', originalBalance: '', interestRate: '', minimumPayment: '', dueDate: '' }); setEditingDebtId(null); setShowDebtForm(false) }
  function openEditDebt(d: Debt) { setDebtForm({ name: d.name, type: d.type, balance: String(d.balance), originalBalance: String(d.originalBalance), interestRate: String(d.interestRate), minimumPayment: String(d.minimumPayment), dueDate: d.dueDate != null ? String(d.dueDate) : '' }); setEditingDebtId(d.id); setShowDebtForm(true) }
  async function submitDebt(e: React.FormEvent) { e.preventDefault(); if (!debtForm.name.trim()) return; const balanceNum = parseFloat(debtForm.balance) || 0; const originalNum = debtForm.originalBalance ? parseFloat(debtForm.originalBalance) : balanceNum; const rateNum = parseFloat(debtForm.interestRate) || 0; const minNum = parseFloat(debtForm.minimumPayment) || 0; const dueNum = debtForm.dueDate ? Math.max(1, Math.min(31, parseInt(debtForm.dueDate, 10))) : undefined; const payload: any = { name: debtForm.name.trim(), type: debtForm.type, balance: balanceNum, originalBalance: originalNum, interestRate: rateNum, minimumPayment: minNum }; if (dueNum !== undefined && !Number.isNaN(dueNum)) payload.dueDate = dueNum; if (editingDebtId) { const res = await fetch('/api/life/finance/debts', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: editingDebtId, updates: payload }) }); const data = await res.json(); if (data.debt) setDebts(prev => prev.map(d => d.id === editingDebtId ? data.debt : d)) } else { const res = await fetch('/api/life/finance/debts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const data = await res.json(); if (data.debt) setDebts(prev => [...prev, data.debt]) } resetDebtForm() }
