              ...
              <div className="flex flex-wrap gap-2">
                {MINDSET_OPTIONS.map(m => (
                  <button key={m} type="button" onClick={() => setMorning(f => ({ ...f, tradingMindset: m }))}
                    ...
                    style={morning.tradingMindset === m ? { background: MINDSET_COLORS[m] + '22', borderColor: MINDSET_COLORS[m], color: MINDSET_COLORS[m] } : { background: 'rgba(255,255,255,0.03)', borderColor: 'var(--border-panel)', color: 'var(--text-muted)' }}>
                    {m}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {MOOD_TAGS.map(tag => (
                  <button key={tag} type="button" onClick={() => toggleMoodTag(tag)}
                    ...
                    style={evening.moodTags.includes(tag) ? { background: MOOD_COLORS[tag] + '22', borderColor: MOOD_COLORS[tag], color: MOOD_COLORS[tag] } : { background: 'rgba(255,255,255,0.03)', borderColor: 'var(--border-panel)', color: 'var(--text-muted)' }}>
                    {tag}
                  </button>
                ))}
              </div>
                  ...
                  <div key={entry.id} className="premium-card overflow-hidden" style={{ borderLeft: `3px solid ${borderColor}` }}>
                    <button onClick={() => setExpandedId(isExpanded ? null : entry.id)}
                      ...
                      <span className="text-xs flex-1 truncate" style={{ color: 'var(--text-muted)', minWidth: 0 }}>
                        {entry.morningFocus ? entry.morningFocus.slice(0, 60) + (entry.morningFocus.length > 60 ? '...' : '')
                          : entry.intention ? entry.intention.slice(0, 60) + (entry.intention.length > 60 ? '...' : '')
