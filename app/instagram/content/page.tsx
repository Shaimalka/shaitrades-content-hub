...
export default function ContentPage() {
 ...
 if (competitors.length === 0) { setFindSummary('No tracked competitors found.'); return }
 ...
 try {
 const scrapeRes = await fetch('/api/competitors/scrape', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ handles: [username] }) })
 ...
 const viralRes = await fetch('/api/competitors/viral-scripts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, posts: newPosts }) })
 if (!viralRes.ok) continue
 const viralData = await viralRes.json()
 const newScripts: ViralScript[] = (viralData.scripts || viralData.viralScripts || []).map((s: any) => ({ ...s, status: 'pending' as const, generatedAt: new Date().toISOString() }))
 if (newScripts.length > 0) { allNewScripts.push(...newScripts); competitorsWithNew++; totalNewScripts += newScripts.length }
 } catch { }
 }
 ...
 } else { setFindSummary('No new content found.') }
 } catch (e) { console.error(e); setFindSummary('Error scanning for new content.') }
 ...
 try {
 const res = await fetch('/api/content/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ context, numIdeas: count, contentType: genStyle === 'all' ? undefined : genStyle }) })
 ...
 try {
 const existing: any[] = JSON.parse(localStorage.getItem(storageKey('schedulerDrafts')) || '[]')
 const draft = { id: Date.now() + Math.random(), contentType: idea.contentType || genStyle || 'Reel', hook: idea.hook, script: idea.script, cta: idea.cta, viralStructure: idea.inspiredBy || '', inspiredBy: context?.competitorUsername || '', originalUrl: '', status: 'scripted', scheduledDate: null, scheduledTime: '23:00', caption: '', createdAt: new Date().toISOString() }
 existing.push(draft)
 localStorage.setItem(storageKey('schedulerDrafts'), JSON.stringify(existing))
 router.push('/instagram/scheduler')
 } catch (e) { console.error(e) }
 ...
 className={`text-xs px-4 py-1.5 font-medium transition-all ${activeTab === tab ? 'bg-white text-black' : 'bg-transparent text-muted hover:border-gray-500'}`}>
 ...
 className={`text-xs px-3 py-1.5 font-medium transition-all ${timeFilter === key ? 'bg-white text-black' : 'text-muted hover:border-gray-500'}`}>
 {label}
 </button>
 ))}
 </div>
 ...
 className="text-xs px-3 py-1.5 text-[var(--text-secondary)] hover:border-white hover:text-white disabled:opacity-50 transition-all">
 ...
 <button onClick={() => router.push('/instagram/competitors')} className="text-xs px-4 py-2 text-[var(--text-secondary)] hover:border-white hover:text-white transition-all">→ Go to Competitors</button>
 </div>
 ) : (
 <>
 <div className="flex items-center justify-between">
 ...
 className={`text-xs px-2.5 py-1 transition-all ${filter === f ? 'bg-white text-black font-medium' : 'text-muted hover:border-gray-500'}`}>
 {f}
 </button>
 ))}
 </div>
 ...
 <p className="text-gray-600 text-sm">No {contentTypeTab === 'all' ? '' : contentTypeTab + ' '}scripts match this filter.</p>
 </div>
 )}
 ...
 const statusColor = script.status === 'scheduled' ? 'text-neon-cyan border-cyan-400/30' : script.status === 'approved' ? 'text-neon-green border-green-400/30' : 'text-muted '
 ...
 return (
 <div key={realIdx} className={`cyber-panel border ${script.status === 'scheduled' ? 'border-cyan-500/20' : script.status === 'approved' ? 'border-green-500/20' : ''} transition-colors`}>
 <div className="p-4">
 ...
 <div className="w-12 h-12 cyber-panel flex-shrink-0 flex items-center justify-center text-gray-700 text-xs">{script.originalPost.type === 'Video' ? '▶' : '🖼'}</div>
 )}
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2 mb-1 flex-wrap">
 <span className={`text-[10px] font-semibold ${typeColor}`}>{normType === 'Reel' ? '🎬' : normType === 'Carousel' ? '🖼' : '📸'} {normType}</span>
 ...
 <p className="text-gray-600 text-xs mt-0.5">{script.shaiRemake.viralStructure}</p>
 </div>
 </div>
 <div className="flex items-center gap-1 flex-shrink-0">
 <button onClick={() => setExpandedId(isExpanded ? null : realIdx)} className="text-xs text-gray-600 hover:text-white transition-colors px-2 py-1">{isExpanded ? '↑' : '↓'}</button>
 <a href={script.originalPost.url} target="_blank" rel="noopener noreferrer" className="text-xs text-gray-700 hover:text-white transition-colors px-2 py-1">↗</a>
 </div>
 </div>
 ...
 {isExpanded && (
 <div className="border-t divide-y divide-[#1a1a1a]">
 <div className="p-4">
 <p className="text-[10px] text-gray-600 uppercase tracking-widest mb-2">📹 Original Post Script (reconstructed)</p>
 <p className="text-muted text-xs leading-relaxed cyber-panel p-3 ">{script.originalPost.reconstructedScript}</p>
 </div>
 <div className="p-4">
 <div className="flex items-center justify-between mb-2">
 <p className="text-[10px] text-gray-600 uppercase tracking-widest">🪝 Hook</p>
 <button onClick={() => copy(script.shaiRemake.hook)} className="text-[10px] text-gray-700 hover:text-white">copy</button>
 </div>
 <p className="text-white text-sm font-medium cyber-panel p-3 ">{script.shaiRemake.hook}</p>
 </div>
 <div className="p-4">
 <div className="flex items-center justify-between mb-2">
 ...
 <div className="text-[var(--text-primary)] text-sm leading-relaxed cyber-panel p-3 whitespace-pre-wrap max-h-64 overflow-y-auto">{script.shaiRemake.script}</div>
 </div>
 <div className="p-4">
 <div className="flex items-center justify-between mb-2">
 ...
 <p className="text-[var(--text-primary)] text-sm cyber-panel p-3 ">{script.shaiRemake.cta}</p>
 </div>
 <div className="p-4 flex gap-2">
 <button onClick={() => copy(script.shaiRemake.hook + '\n\n' + script.shaiRemake.script + '\n\n' + script.shaiRemake.cta)} className="flex-1 py-2 text-[var(--text-secondary)] hover:border-gray-500 hover:text-white text-xs transition-all">Copy Full Script</button>
 ...
 <button onClick={() => approveScript(realIdx)} className="px-4 py-2 bg-white text-black hover:bg-gray-200 text-xs font-bold transition-all">✓ Approve</button>
 </>
 )}
 {script.status === 'approved' && (
 <button onClick={() => sendToScheduler(script)} className="px-6 py-2 bg-cyan-500 text-black hover:bg-cyan-400 text-xs font-bold transition-all">→ Schedule</button>
 )}
 </div>
 </div>
 )}
 ...
 {[3, 5, 10].map(n => (
 <button key={n} onClick={() => setCount(n)} className={`w-9 h-7 text-xs font-medium transition-all ${count === n ? 'bg-white text-black' : 'bg-transparent text-muted hover:border-gray-500'}`}>{n}</button>
 ))}
 ...
 {ideas.map((idea, i) => (
 <div key={i} className="cyber-panel ">
 <div className="p-4 border-b flex items-center gap-3">
 <span className="text-gray-600 text-xs">#{i + 1}</span>
 <span className="text-[10px] text-muted px-1.5 py-0.5">{idea.contentType || genStyle || 'Reel'}</span>
 ...
 <p className="text-white text-sm font-medium cyber-panel p-3 ">{idea.hook}</p>
 </div>
 <div>
 ...
 <div className="text-[var(--text-primary)] text-sm leading-relaxed cyber-panel p-3 whitespace-pre-wrap max-h-48 overflow-y-auto">{idea.script}</div>
 </div>
 <div>
 ...
 <p className="text-[var(--text-primary)] text-sm cyber-panel p-3 ">{idea.cta}</p>
 </div>
 <div className="flex gap-2 pt-1">
 <button onClick={() => copy(idea.hook + '\n\n' + idea.script + '\n\n' + idea.cta)} className="flex-1 py-2 text-muted hover:border-gray-500 hover:text-white text-xs transition-all">Copy Full</button>
