    ...
    try {
      const res = await fetch('/api/content/generate', {
        ...
        body: JSON.stringify({
          context,
          numIdeas: count,
          contentType: genStyle === 'all' ? undefined : genStyle,
        }),
      })
                    ...
                    >
                      ...
                      <div className="p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-3 flex-1 min-w-0">
                            ...
                            {script.originalPost.displayUrl ? (
                              <img
                                src={'/api/proxy-image?url=' + encodeURIComponent(script.originalPost.displayUrl)}
                                alt=""
                                className="w-12 h-12 object-cover flex-shrink-0 opacity-70"
                              />
                            ) : (
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                ...
                                <span className={`text-[10px] px-1.5 py-0.5 border ${statusColor}`}>
                                  {script.status}
                                </span>
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0">
                            <button
                              onClick={() => setExpandedId(isExpanded ? null : realIdx)}
                              className="text-xs text-gray-600 hover:text-white transition-colors px-2 py-1"
                            >
                              {isExpanded ? '↑' : '↓'}
                            </button>
                            <a
                              href={script.originalPost.url}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-xs text-gray-700 hover:text-white transition-colors px-2 py-1"
                            >↗</a>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-4 mt-3 pt-3 border-t border-[#1a1a1a] text-xs text-gray-600">
                          ...
                          {script.originalPost.videoViewCount > 0 && (
                            <span>▶ {fmt(script.originalPost.videoViewCount)}</span>
                          )}
                          <span className="ml-auto text-gray-700 italic truncate max-w-[200px]">
                            {script.originalPost.whyViral}
                          </span>
                        </div>
                      </div>
                        ...
                        <div className="border-t border-[#1a1a1a] divide-y divide-[#1a1a1a]">
                          <div className="p-4">
                            <p className="text-[10px] text-gray-600 uppercase tracking-widest mb-2">
                              📹 Original Post Script (reconstructed)
                            </p>
                          ...
                          <div className="p-4">
                            ...
                            <p className="text-white text-sm font-medium bg-[#080808] p-3 border border-[#1a1a1a]">
                              {script.shaiRemake.hook}
                            </p>
                          </div>
                          <div className="p-4">
                          ...
                          <div className="p-4">
                            ...
                            <p className="text-gray-300 text-sm bg-[#080808] p-3 border border-[#1a1a1a]">
                          ...
                          <div className="p-4 flex gap-2">
                            ...
                            {script.status === 'approved' && (
                              <button
                                onClick={() => sendToScheduler(script)}
        ...
        <div className="space-y-4">
          ...
          {context ? (
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-gray-500 text-xs">
                    {context.topPosts.length} posts loaded · sent {new Date(context.sentAt).toLocaleDateString()}
                  </p>
                </div>
                <button
              ...
              <div className="flex items-center gap-3 mt-3">
                ...
                {[3, 5, 10].map(n => (
                  <button
                    key={n}
                    onClick={() => setCount(n)}
                    className={`w-9 h-7 text-xs font-medium transition-all ${
                      count === n
                        ? 'bg-white text-black'
                        : 'bg-transparent text-gray-500 border border-[#333] hover:border-gray-500'
                    }`}
            ...
            <div className="border border-dashed border-[#222] p-10 text-center">
              ...
              <p className="text-gray-700 text-xs mb-4">
                Go to Competitors → click{' '}
                <strong className="text-gray-500">"Send to Content Gen"</strong> on any analyzed competitor.
              </p>
              <button
                onClick={() => router.push('/instagram/competitors')}
                className="text-xs px-4 py-2 border border-[#333] text-gray-400 hover:border-white hover:text-white transition-all"
              ...
              {ideas.map((idea, i) => (
                <div key={i} className="bg-[#0d0d0d] border border-[#1e1e1e]">
                  <div className="p-4 border-b border-[#1a1a1a] flex items-center gap-3">
                    ...
                    <span className="text-[10px] text-gray-500 border border-[#333] px-1.5 py-0.5">
                      {idea.contentType || genStyle || 'Reel'}
                    </span>
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    <div>
                      ...
                      <div className="text-gray-300 text-sm leading-relaxed bg-[#080808] p-3 border border-[#1a1a1a] whitespace-pre-wrap max-h-48 overflow-y-auto">
                        {idea.script}
                      </div>
                    </div>
                    <div>
