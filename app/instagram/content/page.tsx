...
// Post thumbnail fallback — shows post number + likes since Instagram CDN blocks cross-origin requests
function PostThumbnail({ post, idx }: { post: TopPost; idx: number }) {
    ...
    return (
          <div className="w-14 h-14 bg-[#1a1a1a] border border-[#2a2a2a] flex flex-col items-center justify-center gap-0.5 group-hover:border-[#444] transition-colors">
                <span className="text-gray-500 text-[9px] font-mono">#{idx + 1}</span>span>
            {isVideo ? (
                    <span className="text-gray-500 text-[11px]">▶</span>span>
                  ) : (
                    <span className="text-gray-500 text-[11px]">◻</span>span>
                )}
                <span className="text-gray-500 text-[8px]">{formatNum(post.likesCount)}♥</span>span>
          </div>div>
        )
}
                                                      ...
                                                      <div className="space-y-6">
                                                        {/* Header */}
                                                            <div className="flex items-center justify-between">
                                                                    <div>
                                                                              <h2 className="text-white text-xl font-bold tracking-tight">Content Generator</h2>h2>
                                                                              <p className="text-gray-400 text-sm mt-1">
                                                                                          AI-powered Hook + Script + CTA for every post idea
                                                                              </p>p>
                                                                    </div>div>
                                                              {ideas.length > 0 && (
                                                                  <span className="text-gray-500 text-xs">{ideas.length} ideas generated</span>span>
                                                                    )}
                                                            </div>div>
                                                        ...
                                                        {context ? (
                                                                <div className="bg-[#0d0d0d] border border-[#1e1e1e] p-4">
                                                                          <div className="flex items-start justify-between mb-3">
                                                                                      <div>
                                                                                                    <p className="text-white text-sm font-bold">
                                                                                                                    Context: @{context.competitorUsername}
                                                                                                      </p>p>
                                                                                                    <p className="text-gray-500 text-xs mt-0.5">
                                                                                                      {formatNum(context.followersCount)} followers &middot; {context.topPosts.length} top posts loaded &middot;{' '}
                                                                  ...
                                                                  {/* Top posts preview — uses PostThumbnail fallback (no broken <img> tags) */}
                                                                  ...
                                                                  {error && (
                                                                              <p className="text-red-400 text-xs mt-2">{error}</p>p>
                                                                          )}
                                                                </div>div>
                                                              ) : (
                                                                <div className="bg-[#0d0d0d] border border-dashed border-[#2a2a2a] p-8 text-center">
                                                                          <p className="text-gray-400 text-sm font-medium mb-2">No competitor context loaded</p>p>
                                                                                                            ...
                                                                                                            <div className="flex items-start justify-between gap-3">
                                                                                                                              <div className="flex-1 min-w-0">
                                                                                                                                                  <div className="flex items-center gap-2 mb-1">
                                                                                                                                                                        <span className="text-gray-600 text-xs font-mono">#{idea.ideaNumber || idx + 1}</span>span>
                                                                                                                                                    {idea.contentType && (
                                                                                                                                      <span className="text-[10px] px-1.5 py-0.5 bg-[#1a1a1a] text-gray-400 border border-[#2a2a2a]">
                                                                                                                                        {idea.contentType}
                                                                                                                                                    ...
                                                                                                                                                    {idea.inspiredBy && (
                                                                                                                                      <span className="text-gray-600 text-[10px]">inspired by: {idea.inspiredBy}</span>span>
                                                                                                                                                                        )}
                                                                                                                                                    </div>div>
                                                                                                              ...
                                                                                                              <div className="px-4 pb-4 space-y-4 border-t border-[#1a1a1a] pt-4">
                                                                                                                {/* Hook */}
                                                                                                                                <div>
                                                                                                                                                    ...
                                                                                                                                                    <div className="bg-[#0a0a0a] border border-[#1e1e1e] p-3">
                                                                                                                                                                          <p className="text-white text-sm leading-relaxed font-medium">{idea.hook}</p>p>
                                                                                                                                                      </div>div>
                                                                                                                                  </div>div>
                                                                                                                ...
                                                                                                                {/* Script */}
                                                                                                                                <div>
                                                                                                                                                    ...
                                                                                                                                                    <div className="bg-[#0a0a0a] border border-[#1e1e1e] p-3">
                                                                                                                                                                          <p className="text-gray-300 text-sm leading-relaxed whitespace-pre-wrap">{idea.script}</p>p>
                                                                                                                                                      </div>div>
                                                                                                                                  </div>div>
                                                                                                                ...
                                                                                                                {/* CTA */}
                                                                                                                                <div>
                                                                                                                                                    ...
                                                                                                                                                    <div className="bg-[#0a0a0a] border border-[#1e1e1e] p-3">
                                                                                                                                                                          <p className="text-yellow-400 text-sm leading-relaxed font-medium">{idea.cta}</p>p>
                                                                                                                                                      </div>div>
                                                                                                                                  </div>div>
