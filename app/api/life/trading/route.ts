...
export async function GET(req: NextRequest) {
  ...
  try {
        const logs = await redis.get(tradingKey(userId)) || []
...
export async function POST(req: NextRequest) {
  ...
  try {
        ...
        const logs: any[] = (await redis.get(tradingKey(userId)) as any[]) || []

              if (action === 'delete' && entry?.id) {
                      const updated = logs.filter((l: any) => l.id !== entry.id)
                      await redis.set(tradingKey(userId), updated)
...
export async function PUT(req: NextRequest) {
  ...
  try {
      ...
      const updatedTrade = {
        ...
        await redis.set(tradingKey(userId), updatedLogs)
