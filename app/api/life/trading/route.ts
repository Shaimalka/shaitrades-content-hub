...
export async function PUT(req: NextRequest) {
        ...
        try {
                    ...
                    const pnl = (updatedFields.direction ?? existing.direction) === 'Long'
                        ? ((updatedFields.exitPrice ?? existing.exitPrice) - (updatedFields.entryPrice ?? existing.entryPrice)) * (updatedFields.contracts ?? existing.contracts)
                                    : ((updatedFields.entryPrice ?? existing.entryPrice) - (updatedFields.exitPrice ?? existing.exitPrice)) * (updatedFields.contracts ?? existing.contracts)
                    const updatedTrade = {
                                    ...existing,
                                    ...updatedFields,
                                    pnl: parseFloat(pnl.toFixed(2)),
