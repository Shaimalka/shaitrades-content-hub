...
// ── POST ─────────────────────────────────────────────────────────────────────
// Handles two actions:
//   { action: 'toggle', habitId, date }  — toggle completion for a specific date
//   { entry: { name, stack, … } }        — add a new habit
export async function POST(req: NextRequest) {
  ...
  try {
        ...
        const habits: any[] = ((await redis.get(KEY)) as any[]) || []
            ...
            // Completions: Record<date, Record<habitId, boolean>>
            const completions: Record<string, Record<string, boolean>> =
                      ...
                      if (!completions[date]) completions[date] = {}
                                ...
                                // Toggle: if currently true -> remove, else -> set true
