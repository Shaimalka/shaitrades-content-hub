...
function habitsKey(userId: string) { return `habits:${userId}` }
function completionsKey(userId: string) { return `habits:${userId}:completions` }
...
export async function GET(req: NextRequest) {
  ...
  try {
        const habits = (await redis.get(habitsKey(userId))) || []
              const completions = (await redis.get(completionsKey(userId))) || {}
...
export async function POST(req: NextRequest) {
  ...
  try {
        ...
        const habits: any[] = ((await redis.get(habitsKey(userId))) as any[]) || []
              ...
              if (action === 'toggle') {
                      ...
                      const completions: Record<string, Record<string, boolean>> =
                                ((await redis.get(completionsKey(userId))) as Record<string, Record<string, boolean>>) || {}
                      ...
                      await redis.set(completionsKey(userId), completions)
      ...
      if (entry) {
              ...
              await redis.set(habitsKey(userId), updated)
              const completions = (await redis.get(completionsKey(userId))) || {}
