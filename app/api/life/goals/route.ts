...
import { TrendingUp, Plus, Trash2, Flame } from 'lucide-react'
...
function TradingJournalInner() {
  ...
  useEffect(() => {
    fetch('/api/life/trading')
      .then(r => r.json())
      .then(d => { setTrades(d.logs || []); setLoading(false) })
  ...
  trades.forEach(t => {
    ...
    monthlyData[month] = (monthlyData[month] || 0) + t.pnl
  })
  const chartData = Object.entries(monthlyData).sort().map(([month, pnl]) => ({
    month: month.slice(5),
  ...
  const bestDay = sortedDates.reduce((best, d) => {
    const dayPnl = trades.filter(t => t.date === d).reduce((s, t) => s + t.pnl, 0)
    return dayPnl > best ? dayPnl : best
  }, 0)
  const worstDay = sortedDates.reduce((worst, d) => {
    const dayPnl = trades.filter(t => t.date === d).reduce((s, t) => s + t.pnl, 0)
    return dayPnl < worst ? dayPnl : worst
  }, 0)

  return (
    <div className="cyber-bg-grid min-h-screen">
      <div className="max-w-[1200px] mx-auto p-6 pr-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Link href="/life" className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>← LIFE HUB</Link>
            </div>
          ...
          <div className="flex items-center gap-3">
            {streak > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-mono font-semibold"
        ...
        {showForm && (
          <div className="cyber-panel p-5 mb-6">
            ...
            <form onSubmit={submitTrade} className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div>
                <label className="text-xs font-mono mb-1 block" style={{ color: 'var(--text-muted)' }}>TIME</label>
                <input type="time" value={form.time} onChange={e => setForm(f => ({ ...f, time: e.target.value }))} className="cyber-input w-full" />
              </div>
              <div>
                <label className="text-xs font-mono mb-1 block" style={{ color: 'var(--text-muted)' }}>DIRECTION</label>
                <select value={form.direction} onChange={e => setForm(f => ({ ...f, direction: e.target.value as 'Long' | 'Short' }))} className="cyber-input w-full">
                  <option value="Long">Long</option>
                  <option value="Short">Short</option>
                </select>
              </div>
              <div>
                <label className="text-xs font-mono mb-1 block" style={{ color: 'var(--text-muted)' }}>CONTRACTS</label>
                <input type="number" step="0.01" value={form.contracts} onChange={e => setForm(f => ({ ...f, contracts: e.target.value }))} className="cyber-input w-full" placeholder="1" required />
              </div>
              <div>
                <label className="text-xs font-mono mb-1 block" style={{ color: 'var(--text-muted)' }}>ENTRY PRICE</label>
                <input type="number" step="0.01" value={form.entryPrice} onChange={e => setForm(f => ({ ...f, entryPrice: e.target.value }))} className="cyber-input w-full" placeholder="0.00" required />
              </div>
              <div>
                <label className="text-xs font-mono mb-1 block" style={{ color: 'var(--text-muted)' }}>EXIT PRICE</label>
                <input type="number" step="0.01" value={form.exitPrice} onChange={e => setForm(f => ({ ...f, exitPrice: e.target.value }))} className="cyber-input w-full" placeholder="0.00" required />
              </div>
              <div>
                <label className="text-xs font-mono mb-1 block" style={{ color: 'var(--text-muted)' }}>EMOTION (1-5)</label>
                <div className="flex gap-1">
                  {EMOTIONS.map((em, i) => (
                    <button key={i} type="button" onClick={() => setForm(f => ({ ...f, emotion: i + 1 }))}
                      className={`flex-1 py-1.5 text-lg rounded transition-all ${form.emotion === i + 1 ? 'ring-1 ring-cyan-400 bg-cyan-400/10' : 'opacity-40 hover:opacity-70'}`}>
                      {em}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="cyber-input flex items-center" style={{ fontFamily: 'JetBrains Mono', color: '#00f2ff' }}>
                  {form.entryPrice && form.exitPrice && form.contracts ? (
                    (() => {
                      ...
                      return <span style={{ color: pnl >= 0 ? '#00ff88' : '#ff00e5' }}>${pnl.toFixed(2)}</span>
                    })()
              ...
              <div className="col-span-2 md:col-span-4">
                ...
                <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} className="cyber-input w-full h-20 resize-none" placeholder="What happened? Mistakes? Lessons?" />
              </div>
              <div className="col-span-2 md:col-span-4 flex gap-3">
                <button type="submit" className="btn-cyber-primary">Save Trade</button>
              ...
              <BarChart data={chartData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                ...
                <Bar dataKey="pnl" radius={[4, 4, 0, 0]}>
                  {chartData.map((entry, i) => (
                    <Cell key={i} fill={entry.pnl >= 0 ? '#00ff88' : '#ff00e5'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="cyber-panel overflow-hidden">
          <div className="p-4 border-b" style={{ borderColor: 'var(--border-panel)' }}>
            <h3 className="section-label">TRADE LOG · {trades.length} ENTRIES</h3>
          </div>
          {loading ? (
            <div className="p-8 text-center text-xs font-mono" style={{ color: 'var(--text-muted)' }}>Loading...</div>
          ) : trades.length === 0 ? (
            <div className="p-8 text-center">
              <p className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>No trades logged yet.</p>
              <button onClick={() => setShowForm(true)} className="btn-cyber-primary mt-3">Log Your First Trade</button>
            </div>
          ) : (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b" style={{ borderColor: 'var(--border-panel)' }}>
                    {['DATE', 'TIME', 'DIR', 'ENTRY', 'EXIT', 'QTY', 'P&L', '😊', 'NOTES', ''].map(h => (
                  ...
                  {[...trades].reverse().map(trade => (
                    <tr key={trade.id} className="border-b hover:bg-white/[0.02] transition-colors" style={{ borderColor: 'var(--border-subtle)' }}>
                      ...
                      <td className="px-4 py-3">
                        <span className="badge-pill" style={trade.direction === 'Long'
                          ? { color: '#00ff88', borderColor: 'rgba(0,255,136,0.3)', background: 'rgba(0,255,136,0.08)' }
                          : { color: '#ff00e5', borderColor: 'rgba(255,0,229,0.3)', background: 'rgba(255,0,229,0.08)' }}>
                          {trade.direction}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 font-mono font-bold" style={{ color: trade.pnl >= 0 ? '#00ff88' : '#ff00e5' }}>
                        ${trade.pnl.toFixed(2)}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => deleteTrade(trade.id)} className="opacity-30 hover:opacity-70 transition-opacity">
      ...
      <LifeHubChat
        ...
        systemPrompt="You are a trading AI analyst. Analyze the user's trade log."
