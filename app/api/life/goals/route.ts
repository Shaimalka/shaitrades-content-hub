...
// Trade shape from trading:${userId}:logs
type TradeLog = {
    id: string
    pnl?: number
    direction?: string
...
function goalsKey(userId: string) { return `goals:${userId}` }
function tradingKey(userId: string) { return `trading:${userId}:logs` }
...
export async function GET(req: NextRequest) {
  ...
  try {
        const goals = ((await redis.get(goalsKey(userId))) as Goal[]) || []

              // Load this user's trades for trading goal metric calculation
              const allTrades = ((await redis.get(tradingKey(userId))) as TradeLog[]) || []
...
export async function POST(req: NextRequest) {
  ...
  try {
        ...
        const goals: Goal[] = ((await redis.get(goalsKey(userId))) as Goal[]) || []
              const updated = [...goals, newGoal]
        await redis.set(goalsKey(userId), updated)
