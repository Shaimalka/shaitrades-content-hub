...
export async function POST(req: NextRequest) {
  ...
  try {
          ...
          const topPostsSummary = topPosts.slice(0, 10).map((p: any, i: number) => {
                    return `Post #${i + 1} (${p.type || 'Image'}) â ${p.likesCount} likes, ${p.commentsCount} comments${p.videoViewCount ? `, ${p.videoViewCount} views` : ''}
                    Caption: ${(p.caption || '(no caption)').slice(0, 200)}`
          }).join('\n\n')
          const prompt = `You are writing content for Shai â a real, raw, and honest day trader based in Thailand who trades US stocks from Southeast Asia.

          Here is everything you need to know about Shai to write in his voice:
          ...
          - Built this from scratch with zero background in finance â self-taught, figured it out the hard way
          - Brand pillars: honest, raw, vulnerable, real â he shows LOSSES just as much as wins
          ...
          - Relatable to beginners â avoids jargon, explains things simply
          ...
          Now, you've analyzed a competitor account to understand what content formats and hooks perform well. Use that as structural inspiration â but make every idea sound like SHAI, not a generic trading influencer.
          ...
          Based on what performs best for this competitor, generate ${numIdeas} ORIGINAL content ideas written in Shai's voice.
                ...
                "ideaNumber": 1,
                    "contentType": "Reel/Carousel/Single Image",
                        "inspiredBy": "brief note on which competitor post inspired the format/structure",
                            "hook": "The exact first line/visual hook to stop the scroll â make it punchy and specific. Should sound like Shai â personal, real, not a listicle teaser.",
                                "script": "Full word-for-word post script written in Shai's voice. For a reel this is the voiceover. For a carousel, write out each slide. For a single post, write the full caption including line breaks. Make it complete and ready to use â minimum 100 words. Must feel personal, honest, and real â not like a generic finance post.",
                                    "cta": "The exact call-to-action line to end with â specific, compelling, and in Shai's voice (not corporate-sounding)"
                                      }
                                      ]

                                      RULES:
                                      - Every piece of content must sound like SHAI â personal, honest, raw
                                      - Do NOT write generic trading tips content â write Shai's actual lived experience
                                      - Hook must be scroll-stopping â use a real moment, a real number, or a vulnerable admission
                                      - Script must be COMPLETE and WORD-FOR-WORD ready to use (at least 100 words)
                                      - Show the human behind the trades â the emotions, the late nights, the grind
