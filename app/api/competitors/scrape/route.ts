...
export async function POST(request: Request) {
  try {
    ...
    for (const chunk of chunks) {
      ...
      // Log available fields on the first item for debugging
      if (items.length > 0) {
        const firstItem = items[0]
        console.log('[scrape] First item keys:', Object.keys(firstItem))
        console.log('[scrape] First item owner fields:', {
          ownerUsername: firstItem.ownerUsername,
          ownerFollowersCount: firstItem.ownerFollowersCount,
          ownerFollowingCount: firstItem.ownerFollowingCount,
          ownerPostsCount: firstItem.ownerPostsCount,
      ...
      // Group posts by ownerUsername — filter to only requested usernames (case-insensitive)
      ...
      for (const item of items) {
        ...
        // Only include posts whose ownerUsername exactly matches a requested username
      ...
      const chunkCompetitors = Object.entries(byOwner).map(([ownerUsername, posts]) => {
        ...
        // Resolve follower count — try all known field variants
        const followersCount =
          first.ownerFollowersCount ??
          first.owner?.followersCount ??
          first.owner?.follower_count ??
          first.owner?.followers ??
          0

        const followingCount =
          first.ownerFollowingCount ??
          first.owner?.followingCount ??
          first.owner?.following_count ??
          first.owner?.following ??
          0

        console.log(`[scrape] ${ownerUsername} followersCount resolved to: ${followersCount} (raw ownerFollowersCount=${first.ownerFollowersCount}, owner?.followersCount=${first.owner?.followersCount})`)
