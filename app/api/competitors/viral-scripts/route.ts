...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Fix: extract competitor and followerCount robustly ──────────────────
    // The client sends { competitor: CompetitorData, posts: Post[] }
    // followerCount may live on competitor.followersCount or competitor.followerCount
    const competitor = body.competitor || {}
    const followerCount: number =
      Number(competitor.followersCount) ||
      Number(competitor.followerCount) ||
      Number(competitor.followers_count) ||
      0
    ...
    // If no reels/carousels found, fall back to all posts rather than erroring
    const candidatePosts = reelsAndCarousels.length > 0 ? reelsAndCarousels : posts

    // ── Fix: ER filter with fallback ─────────────────────────────────────────
    // If followerCount is 0 or missing → skip ER filter entirely, use top posts by likes
    // If followerCount is known → apply 1% threshold (down from 2.5%)
    let filtered: any[]
    if (!followerCount) {
      // No follower data — sort by raw engagement and take top posts
      filtered = [...candidatePosts].sort(
        (a, b) => ((b.likesCount || 0) + (b.commentsCount || 0)) - ((a.likesCount || 0) + (a.commentsCount || 0))
      )
    } else {
      // Apply 1% ER threshold (large accounts rarely exceed 2.5%)
      filtered = candidatePosts.filter((p: any) => {
        const engagementRate = ((p.likesCount || 0) + (p.commentsCount || 0)) / followerCount * 100
        return engagementRate >= 1.0
      })

      // Safety fallback: if ER filter wipes everything out, use raw likes ranking
      if (!filtered.length) {
        console.warn(`[viral-scripts] ER threshold eliminated all posts for @${competitor.username} (followerCount=${followerCount}). Falling back to top-by-likes.`)
        filtered = [...candidatePosts].sort(
          (a, b) => ((b.likesCount || 0) + (b.commentsCount || 0)) - ((a.likesCount || 0) + (a.commentsCount || 0))
        )
      }
    ...
    if (!filtered.length) {
      return NextResponse.json(
        { error: 'No posts available to generate scripts from.' },
        { status: 400 }
      )
    }
    ...
    const postsContext = top10.map((p: any, i: number) => {
      ...
      const engRate = followerCount
        ? (((p.likesCount || 0) + (p.commentsCount || 0)) / followerCount * 100).toFixed(1)
        : 'N/A'
