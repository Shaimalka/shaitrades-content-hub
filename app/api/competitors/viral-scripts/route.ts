...
export async function POST(req: NextRequest) {
  ...
  try {
        ...
        const reelsAndCarousels = posts.filter((p: any) => {
                ...
                return (
                          type === 'reel' || type === 'video' || type === 'carousel' ||
                          type === 'sidecar' || type === 'graphvideo' || type === 'graphsidecar'
                        )
        })
        ...
        if (!filtered.length) {
                return NextResponse.json({ error: 'No posts available to generate scripts from.' }, { status: 400 })
        }
