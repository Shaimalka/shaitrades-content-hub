...
function loadViralQueue(): ViralScript[] {
  try {
    const s = localStorage.getItem('tiktokViralScriptsQueue')
    return s ? JSON.parse(s) : []
  } catch { return [] }
}

function saveViralQueue(q: ViralScript[]) {
  localStorage.setItem('tiktokViralScriptsQueue', JSON.stringify(q))
}
...
export default function TikTokContentPage() {
  ...
  useEffect(() => {
    ...
    try {
      const saved = localStorage.getItem('tiktokContentGenContext')
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch('/api/content/generate', {
        ...
        body: JSON.stringify({ context, numIdeas: count, contentType: 'TikTok Video' }),
      })
  ...
  const sendIdeaToScheduler = (idea: GeneratedIdea) => {
    try {
      ...
      const draft = {
        id: Date.now() + Math.random(),
        contentType: idea.contentType || 'TikTok Video',
        hook: idea.hook, script: idea.script, cta: idea.cta,
        viralStructure: idea.inspiredBy || '',
        inspiredBy: context?.competitorUsername || '',
        originalUrl: '', status: 'scripted',
        scheduledDate: null, scheduledTime: '23:00', caption: '',
        createdAt: new Date().toISOString(),
      }
      existing.push(draft)
      localStorage.setItem('schedulerDrafts', JSON.stringify(existing))
  ...
  const filteredQueue = filter === 'all' ? queue : queue.filter(s => s.status === filter)

  return (
    <div className="space-y-6 p-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">TikTok Content Generator</h1>
          <p className="text-gray-500 text-sm mt-0.5">AI-powered TikTok hooks, scripts, and CTAs</p>
        </div>
        ...
        <div className="space-y-4">
          {queue.length === 0 ? (
            <div className="border border-dashed border-[#222] p-12 text-center">
              ...
              <p className="text-gray-600 text-sm mb-3">No TikTok viral scripts yet.</p>
              <p className="text-gray-700 text-xs mb-4">
                Go to <strong className="text-gray-500">TikTok Competitors</strong>, pick a tracked account, and click <strong className="text-gray-500">🔥 Viral Scripts</strong>.
              </p>
              <button
                onClick={() => router.push('/tiktok/competitors')}
                className="text-xs px-4 py-2 border border-[#333] text-gray-400 hover:border-white hover:text-white transition-all"
              >
                → Go to TikTok Competitors
              </button>
            </div>
          ) : (
            <>
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {scheduledCount > 0 && (
                    <button onClick={clearScheduled} className="text-[10px] text-gray-600 hover:text-red-400 transition-colors">clear scheduled</button>
                  )}
              ...
              <div className="space-y-3">
                {filteredQueue.map((script) => {
                  ...
                  const statusColor = script.status === 'scheduled'
                  ...
                  return (
                    <div
                      key={realIdx}
                      className={`bg-[#0d0d0d] border ${script.status === 'scheduled' ? 'border-cyan-500/20' : script.status === 'approved' ? 'border-green-500/20' : 'border-[#1e1e1e]'} transition-colors`}
                    >
                      <div className="p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-3 flex-1 min-w-0">
                            {script.originalPost.displayUrl ? (
                              ...
                              <div className="w-12 h-12 bg-[#1a1a1a] flex-shrink-0 flex items-center justify-center text-gray-700 text-xs">▶</div>
                            )}
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                <span className="text-[10px] font-semibold text-pink-400">🎵 TikTok</span>
                        ...
                        <div className="flex gap-4 mt-3 pt-3 border-t border-[#1a1a1a] text-xs text-gray-600">
                          <span>▶ {fmt(script.originalPost.videoViewCount)}</span>
                      ...
                      {isExpanded && (
                        <div className="border-t border-[#1a1a1a] divide-y divide-[#1a1a1a]">
                          <div className="p-4">
                            <p className="text-[10px] text-gray-600 uppercase tracking-widest mb-2">📹 Original TikTok Script (reconstructed)</p>
                          ...
                          <div className="p-4">
                            <div className="flex items-center justify-between mb-2">
                              <p className="text-[10px] text-gray-600 uppercase tracking-widest">🪝 Hook (first 2 seconds)</p>
                          ...
                          {script.shaiRemake.captionWithHashtags && (
                            <div className="p-4">
                              <div className="flex items-center justify-between mb-2">
                                <p className="text-[10px] text-gray-600 uppercase tracking-widest"># Caption + Hashtags</p>
                                <button onClick={() => copy(script.shaiRemake.captionWithHashtags!)} className="text-[10px] text-gray-700 hover:text-white">copy</button>
                              </div>
                              <p className="text-gray-500 text-xs bg-[#080808] p-3 border border-[#1a1a1a]">{script.shaiRemake.captionWithHashtags}</p>
                            </div>
                          )}
                          <div className="p-4 flex gap-2">
                            <button
                              onClick={() => copy(`${script.shaiRemake.hook}\n\n${script.shaiRemake.script}\n\n${script.shaiRemake.cta}`)}
      ...
      {activeTab === 'generate' && (
        <div className="space-y-4">
            ...
            <div className="bg-[#111] border border-[#222] p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-gray-500 text-xs">{context.topPosts.length} TikTok videos loaded · sent {new Date(context.sentAt).toLocaleDateString()}</p>
                </div>
                <button
                  onClick={() => { localStorage.removeItem('tiktokContentGenContext'); setContext(null); setIdeas([]) }}
              ...
              <div className="flex items-center gap-3 mt-3">
                ...
                <button onClick={generate} disabled={loading} className="flex-1 py-1.5 bg-white hover:bg-gray-200 disabled:opacity-50 text-black text-xs font-bold transition-all">
                  {loading ? 'Generating...' : '⚡ Generate TikTok Ideas'}
                </button>
              </div>
            </div>
          ) : (
            <div className="border border-dashed border-[#222] p-10 text-center">
              <p className="text-gray-600 text-sm mb-3">No TikTok competitor context loaded.</p>
              <p className="text-gray-700 text-xs mb-4">Go to TikTok Competitors → click <strong className="text-gray-500">"Send to Content Gen"</strong> on any account.</p>
              <button onClick={() => router.push('/tiktok/competitors')} className="text-xs px-4 py-2 border border-[#333] text-gray-400 hover:border-white hover:text-white transition-all">
                → Go to TikTok Competitors
              </button>
            </div>
          )}
          {ideas.length > 0 && (
            <div className="space-y-3">
              <p className="text-gray-600 text-[10px] uppercase tracking-widest">{ideas.length} generated TikTok ideas</p>
              {ideas.map((idea, i) => (
                <div key={i} className="bg-[#0d0d0d] border border-[#1e1e1e]">
                  <div className="p-4 border-b border-[#1a1a1a] flex items-center gap-3">
                    ...
                    <span className="text-[10px] text-pink-400 border border-[#333] px-1.5 py-0.5">🎵 TikTok Video</span>
                  </div>
                  <div className="p-4 space-y-3">
                    <div>
                      <div className="flex justify-between mb-1">
                        <span className="text-[10px] text-gray-600 uppercase tracking-widest">Hook</span>
                        <button onClick={() => copy(idea.hook)} className="text-[10px] text-gray-700 hover:text-white">copy</button>
                      </div>
                      <p className="text-white text-sm font-medium bg-[#080808] p-3 border border-[#1a1a1a]">{idea.hook}</p>
                    </div>
                    <div>
                      <div className="flex justify-between mb-1">
                        <span className="text-[10px] text-gray-600 uppercase tracking-widest">Script</span>
                        <button onClick={() => copy(idea.script)} className="text-[10px] text-gray-700 hover:text-white">copy</button>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between mb-1">
                        <span className="text-[10px] text-gray-600 uppercase tracking-widest">CTA</span>
                        <button onClick={() => copy(idea.cta)} className="text-[10px] text-gray-700 hover:text-white">copy</button>
                      </div>
