    ...
    if (!active || !payload?.length) return null
        return (
              <div className="bg-[#111] border border-[#333] px-3 py-2 text-xs">
                    <p className="text-gray-400 mb-1">{label}</p>p>
                {payload.map((p: any, i: number) => (
                        <p key={i} style={{ color: p.color }}>{p.name}: {p.value}%</p>p>
                      ))}
              </div>div>
            )
}
...
function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
    return (
          <div className="bg-[#0d0d0d] border border-[#1e1e1e] p-4">
                ...
                <p className="text-white text-2xl font-bold">{value}</p>p>
            {sub && <p className="text-gray-600 text-xs mt-1">{sub}</p>p>}
          </div>div>
        )
}
          ...
          <div className="p-6 space-y-8 max-w-6xl">
            {/* Header */}
                <div className="flex items-center justify-between">
                        <div>
                                  <h1 className="text-white text-xl font-bold tracking-tight">TikTok Analytics</h1>h1>
            ...
            {loading && (
                    <div className="space-y-4">
                              ...
                              <p className="text-gray-600 text-xs text-center">Scraping @{HANDLE} TikTok... this takes ~2-3 min</p>p>
                    </div>div>
                )}
                    ...
                    <div className="space-y-8">
                      ...
                      {bestVideo && (
                                  <div className="bg-[#0d0d0d] border border-cyan-500/30 p-5">
                                                <div className="flex items-center gap-2 mb-4">
                                                                <span className="text-cyan-400 text-xs font-bold uppercase tracking-widest">#1 Best Performing Video</span>span>
                                                </div>div>
                                                                ...
                                                                <div className="flex flex-col justify-between flex-1">
                                                                                  <div>
                                                                                                      ...
                                                                                                      <div className="flex gap-4 text-xs">
                                                                                                                            <span className="text-cyan-400 font-bold">{fmt(bestVideo.videoViewCount)} views</span>span>
                                                                                                                            <span className="text-amber-400">♥ {fmt(bestVideo.likesCount)}</span>span>
                                                                                                                            <span className="text-emerald-400">💬 {fmt(bestVideo.commentsCount)}</span>span>
                                                                                                        </div>div>
                                                                                    </div>div>
                                                                                  <a
                                                                                                        href={bestVideo.url}
                      ...
                      {/* ── Views & Engagement bar chart ── */}
                              <div className="bg-[#0d0d0d] border border-[#1e1e1e] p-5">
                                          <h2 className="text-white text-sm font-bold mb-1">Views & Engagement per Post</h2>h2>
                                          <p className="text-gray-500 text-xs mb-4">Top 10 posts by views</p>p>
                                          <div className="h-64">
                      ...
                      {/* ── Engagement Rate per Video ── */}
                              <div className="bg-[#0d0d0d] border border-[#1e1e1e] p-5">
                                          <h2 className="text-white text-sm font-bold mb-1">Engagement Rate per Video</h2>h2>
                                          <p className="text-gray-500 text-xs mb-4">((likes + comments) / views × 100) — top 10 posts</p>p>
                                          <div className="h-52">
                                                        <ResponsiveContainer width="100%" height="100%">
                      ...
                      {growthData.length > 1 && (
                                  <div className="bg-[#0d0d0d] border border-[#1e1e1e] p-5">
                                                <h2 className="text-white text-sm font-bold mb-1">Cumulative Views Over Time</h2>h2>
                                                <p className="text-gray-500 text-xs mb-4">Based on post publish dates (proxy for account growth)</p>p>
                                                <div className="h-52">
                                                                <ResponsiveContainer width="100%" height="100%">
                      ...
                      {/* ── Top videos grid ── */}
                              <div>
                                          <h2 className="text-white text-sm font-bold mb-1">Top Performing Videos</h2>h2>
                                          <p className="text-gray-500 text-xs mb-4">Sorted by views</p>p>
                                      ...
                                      <a
                                                        ...
                                                        <div className="relative aspect-[9/16] bg-[#111] overflow-hidden">
                                                          {post.displayUrl ? (
                                                                                ...
                                                                                <div className="w-full h-full flex items-center justify-center text-gray-700 text-3xl">▶</div>div>
                                                                            )}
                                                                            <div className="absolute top-1.5 left-1.5 bg-black/70 text-white text-[9px] font-bold px-1.5 py-0.5">
                                                                                                  #{idx + 1}
                                                                            </div>div>
                                                                            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2">
                                                                                                  <p className="text-white text-[10px] font-bold">{fmt(post.videoViewCount)} views</p>p>
                                                                            </div>div>
                                                        </div>div>
                                                        <div className="p-2">
                                                                            ...
                                                                            <div className="flex gap-2 text-[9px] text-gray-600">
                                                                                                  <span>♥ {fmt(post.likesCount)}</span>span>
                                                                                                  <span>💬 {fmt(post.commentsCount)}</span>span>
                                                                            </div>div>
                                                        </div>div>
                                      </a>a>
                                    ))}
                      ...
                      {/* ── Best posting times heatmap ── */}
                              <div className="bg-[#0d0d0d] border border-[#1e1e1e] p-5">
                                          ...
                                          <p className="text-gray-500 text-xs mb-4">Day × hour heatmap — intensity based on total views at that time slot</p>p>
                                        ...
                                        <div key={day} className="flex mb-0.5 items-center">
                                                            <div className="w-10 shrink-0 text-[10px] text-gray-500 pr-1 text-right">{day}</div>div>
                                          {Array.from({ length: 24 }, (_, h) => {
                                                                const val = heatmapData.grid[d]?.[h] || 0
                                                                        ...
                                                                        <div className="flex items-center gap-2 mt-3 justify-end">
                                                                                          <span className="text-[9px] text-gray-600">Less</span>span>
                                          ...
                                          <div
                                                                ...
                                                                />
                                                                                          ...
                                                                                          <span className="text-[9px] text-gray-600">More</span>span>
                                                                        </div>div>
                                                        </div>div>
                                          </div>div>
                              </div>div>
                      ...
                      {/* ── AI Weekly Insight ── */}
                              <div className="bg-[#0d0d0d] border border-[#1e1e1e] p-5">
                                          <div className="flex items-center justify-between mb-4">
                                                        <div>
                                                                        <h2 className="text-white text-sm font-bold">AI Weekly Insight</h2>h2>
                                                                        <p className="text-gray-500 text-xs mt-0.5">Powered by Claude — analyzes your last 7 days of posts</p>p>
                                                        </div>div>
                                                        <button
                                    ...
                                    <div className="bg-[#111] border border-[#222] p-4 space-y-3">
                                      {aiInsight.map((bullet, i) => (
                                                        <div key={i} className="flex gap-3">
                                                                            ...
                                                                            <p className="text-gray-300 text-sm leading-relaxed">{bullet.replace(/^•\s*/, '')}</p>p>
                                                        </div>div>
                                                      ))}
                                    </div>div>
                                          )}
                                ...
                                {!aiInsight && !aiLoading && !aiError && (
                                    <p className="text-gray-600 text-xs">Click "Get AI Insight" to analyze your recent posts with Claude.</p>p>
                                          )}
                              </div>div>
