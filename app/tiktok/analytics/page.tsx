...
export default function TikTokAnalyticsPage() {
  ...
  // 1. Follower growth proxy: cumulative engagement over time (posts sorted by date)
  const growthData = (() => {
  ...
  // 3. Best posting times heatmap: day x hour
  const heatmapData = (() => {
    if (!data?.posts.length) return { grid: [], max: 1 }
    // grid[day][hour] = total views
  ...
  // 4. Top 8 videos for thumbnail grid
  ...
  return (
    <div className="p-6 space-y-8 max-w-6xl">
      ...
      {/* Dashboard — only shown when data loaded */}
      {data && !loading && (
