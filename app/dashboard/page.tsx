  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', color: '#f0f4ff', fontFamily: 'Inter, sans-serif' }}>
      <style dangerouslySetInnerHTML={{ __html: `
        .dash-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(0,242,255,0.08); border-radius: 12px; padding: 20px; transition: border-color 0.2s, box-shadow 0.2s; }
        .dash-card:hover { border-color: rgba(0,242,255,0.18); box-shadow: 0 0 20px rgba(0,242,255,0.06); }
        .quick-link { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 10px; padding: 14px 16px; text-decoration: none; transition: all 0.2s; display: flex; flex-direction: column; gap: 4px; }
        .quick-link:hover { background: rgba(255,255,255,0.06); border-color: rgba(0,242,255,0.2); transform: translateY(-1px); }
        .trade-row { display: flex; align-items: center; gap: 12px; padding: 10px 14px; border-radius: 8px; transition: background 0.15s; }
        .trade-row:hover { background: rgba(255,255,255,0.03); }
        .stat-pill { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px 16px; border-radius: 12px; text-align: center; }
      `}} />
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '36px 32px' }}>
        ...
        <div className="dash-card" style={{ marginBottom: '24px', borderColor: 'rgba(0,242,255,0.15)', background: 'rgba(0,242,255,0.03)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
            <div style={{ width: '44px', height: '44px', borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg, #00f2ff22, #0060ff22)', border: '1px solid rgba(0,242,255,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Georgia, serif', fontWeight: 700, color: '#00f2ff', fontSize: '1rem' }}>S</div>
            <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
          ...
          <div className="dash-card stat-pill" style={{ borderColor: 'rgba(0,255,136,0.12)' }}>
            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '9px', color: 'rgba(255,255,255,0.3)', letterSpacing: '3px', textTransform: 'uppercase', marginBottom: '8px' }}>Monthly PnL</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
          <div className="dash-card">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              ...
              <Link href="/life/trading" style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '9px', color: '#00f2ff', letterSpacing: '1px', textDecoration: 'none', opacity: 0.6 }}>VIEW ALL</Link>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              {recentTrades.map((trade, i) => (
                <div key={i} className="trade-row">
                  <div style={{ width: '6px', height: '6px', borderRadius: '50%', flexShrink: 0, background: trade.result === 'WIN' ? '#00ff88' : '#ff4d6d', boxShadow: trade.result === 'WIN' ? '0 0 6px #00ff88' : '0 0 6px #ff4d6d' }} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {habits.map((item, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ width: '18px', height: '18px', borderRadius: '5px', flexShrink: 0, border: `1.5px solid ${item.done ? item.color : 'rgba(255,255,255,0.12)'}`, background: item.done ? `${item.color}22` : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {item.done && (
                      <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                        <path d="M2 5l2 2 4-4" stroke={item.color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    )}
                  </div>
