...
const renderAnalysis = (text: string) => {
 ...
 timeFilter === f
 ? 'bg-white text-black'
 : 'bg-transparent text-muted hover:border-gray-500'
 }`}
 >
 ...
 <span className="text-muted text-xs">{competitors.length} tracked</span>
 <span className="text-gray-600 text-[10px] px-2 py-0.5 flex items-center gap-1">
 ...
 {/* Add Competitors */}
 <div className="cyber-panel rounded-none p-4">
 <div className="flex gap-2 mb-4">
 <button
 onClick={() => setMode('single')}
 className={`text-xs px-3 py-1.5 font-medium transition-all ${
 mode === 'single' ? 'bg-white text-black' : 'bg-transparent text-gray-400 hover:border-gray-500'
 }`}
 >Single Handle</button>
 <button
 onClick={() => setMode('bulk')}
 className={`text-xs px-3 py-1.5 font-medium transition-all ${
 mode === 'bulk' ? 'bg-white text-black' : 'bg-transparent text-gray-400 hover:border-gray-500'
 }`}
 >Paste List</button>
 </div>
 ...
 className="flex-1 cyber-panel text-white text-sm px-3 py-2 placeholder-gray-600 focus:outline-none focus:border-gray-500 rounded-none"
 />
 ...
 rows={4}
 className="w-full cyber-panel text-white text-sm px-3 py-2 placeholder-gray-600 focus:outline-none focus:border-gray-500 rounded-none resize-none"
 />
 ...
 return (
 <div key={comp.username} className="cyber-panel hover: transition-colors">
 <div className="p-4">
 {comp.isLoading ? (
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 cyber-panel animate-pulse" />
 <div className="flex-1 space-y-2">
 <div className="h-3 cyber-panel animate-pulse w-32" />
 <div className="h-2 cyber-panel animate-pulse w-48" />
 </div>
 ...
 <div className="w-10 h-10 cyber-panel rounded-full flex items-center justify-center">
 <span className="text-muted text-sm">{comp.username[0]?.toUpperCase()}</span>
 </div>
 )}
 <div>
 ...
 {comp.fullName && <p className="text-muted text-xs">{comp.fullName}</p>}
 </div>
 </div>
 ...
 className="text-xs px-3 py-1.5 text-neon-cyan hover:border-cyan-400 font-medium transition-all"
 ...
 className="text-xs px-3 py-1.5 text-orange-400 hover:border-orange-400 font-medium disabled:opacity-40 disabled:cursor-not-allowed transition-all"
 ...
 {comp.biography && (
 <p className="text-muted text-xs mt-2 leading-relaxed line-clamp-2">{comp.biography}</p>
 )}
 ...
 <p className="text-muted text-xs font-medium uppercase tracking-wider">Top Posts</p>
 <button
 ...
 <div className="w-16 h-16 cyber-panel overflow-hidden">
 {post.displayUrl ? (
 ...
 return (
 <div key={post.id || idx} className={`flex items-center gap-3 px-3 py-2 transition-colors ${isOutperformer ? 'bg-green-950/20 border-l-2 border-green-500' : 'cyber-panel hover:cyber-panel'}`}>
 ...
 return (
 <div key={i} className="cyber-panel border border-green-500/20 p-3">
 ...
 <div className="cyber-panel p-2 mb-2">
 ...
 {comp.analysis && (
 <div>
 <p className="text-muted text-xs font-medium uppercase tracking-wider mb-2">🧠 AI Analysis</p>
 ...
 <button
 onClick={() => sendTopPostsToContentGen(comp)}
 className="w-full py-2.5 border border-cyan-500/40 text-neon-cyan text-sm font-medium hover:bg-cyan-500/10 transition-all"
 >
 ...
 <p className="text-muted text-xs mt-0.5">Trading accounts growing fast right now</p>
 </div>
 <button
 onClick={loadRisingAccounts}
 disabled={loadingRising}
 className="text-xs px-4 py-2 text-gray-300 hover:border-white hover:text-white disabled:opacity-40 transition-all font-medium"
 >
 ...
 {[1, 2, 3].map(i => (
 <div key={i} className="cyber-panel p-4 space-y-2">
 <div className="h-3 cyber-panel animate-pulse w-24" />
 <div className="h-2 cyber-panel animate-pulse w-full" />
 <div className="h-2 cyber-panel animate-pulse w-3/4" />
 </div>
 ))}
 </div>
 )}
 ...
 <div key={i} className="cyber-panel hover: transition-colors p-4">
 <div className="flex items-start justify-between">
 ...
 <span className="text-muted text-xs">{formatNum(acc.estimatedFollowers)} followers</span>
 <span className="text-gray-700 text-xs">·</span>
 <span className="text-muted text-xs">{acc.niche}</span>
 </div>
 ...
 <p className="text-muted text-xs leading-relaxed"><span className="text-gray-700">Weakness: </span>{acc.weakness}</p>
 </div>
 <button
 onClick={() => { setInputValue(acc.username); setMode('single'); window.scrollTo({ top: 0, behavior: 'smooth' }) }}
 className="ml-3 flex-shrink-0 text-xs px-3 py-1.5 text-gray-400 hover:border-white hover:text-white transition-all"
 >
 Track →
 </button>
 </div>
 ...
 {risingAccounts.length === 0 && !loadingRising && (
 <div className="text-center py-8 text-gray-600 text-sm border border-dashed ">
